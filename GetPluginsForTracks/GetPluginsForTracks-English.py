...
def getTrackInfo(track):
	...
	mixerStrip = mainWindow.findFirstR(AXRole='AXGroup', AXRoleDescription='channel strip group')
	print 'Found mixer strip: ' + mixerStrip.AXDescription

	# Last group is the generator (instrument)
	generatorGroup = mixerStrip.findAll(AXRole='AXGroup', AXRoleDescription='group')[-1]
	...
	# Get audio effects
	audioEffects = []
	effectsGroups = mixerStrip.findAllR(AXRole='AXGroup', AXRoleDescription='group', AXTitle='Audio FX')
...
def getCleanTrackName(trackDescription):
	# Convert "Track 1 “Piano”" to "Track 1 - Piano"
	return trackDescription.replace(u'\u201c', '- ').replace(u'\u201d', '')
...
tracks = []
tracksContainer = mainWindow.findFirstR(AXRole='AXGroup', AXRoleDescription='group', AXDescription=u'Tracks header')
